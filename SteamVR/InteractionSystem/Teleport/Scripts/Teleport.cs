...
namespace Valve.VR.InteractionSystem
{
	...
	public class Teleport : MonoBehaviour
	{
		...
		void Update()
		{
			...
			foreach ( Hand hand in player.hands )
			{
				if ( visible )
				{
					...
					{
						if ( pointerHand == hand ) //This is the leftPointer hand
						{
							TryTeleportPlayer();
						}
					}
			...
			else
			{
				...
				else if ( visible )
				{
					...
					{
						//Hide the leftPointer
						HidePointer();
					}
					else if ( newPointerHand != null )
					{
						//Move the leftPointer to a new hand
						ShowPointer( newPointerHand, oldPointerHand );
					}
				}
			}
		...
		private void UpdatePointer()
		{
			...
			//Trace to see if the leftPointer hit anything
			RaycastHit hitInfo;
